...
import delSelectedCons from '@salesforce/apex/CandidatePrefLocation_Controller.deleteContacts';
...
export default class DeleteRowsInDatatableLWC extends LightningElement { 
    ...
    @wire(displayContactRecord,{recordId : '$recordId'})
    contacts(result) {
        debugger;
        this.refreshTable = result;
        if (result.data) {
            this.data = result.data.map(row=>{
                return{...row, City: row.City__r.Name,County: row.County__r.Name}
            })
