...
$(document).ready(function() {
    ...
    let configureCalendar = function() {
        $('#calendar').fullCalendar({
            ...
            navLinks: true,
            editable: (!isRDView || isRDView == '' || isRDView === 'false') ? true : false,
            eventLimit: true,
    ...
    let initCal = () => {
        if(contactId !== '') {
            ...
            $("#spinner").show();
            EditCandidateShiftExtention.getInit(contactId,function(result, event) {
                console.log(result);
                debugger;
                //$('#calendar-card').hide();
    ...
    $("#create-new-slot").click(function() {
        ...
        if(startDateText !== "" && endDateText !== "") {
            ...
            $("#spinner").show();
            EditCandidateShiftExtention.createSlot(slot__c, startTime, endTime, JSON.stringify(selection), function(result, event) {
                if(event.status) {
                    debugger;
                    if(result) {
                        if(result === 'SUCCESS') {
    ...
    $("#delete-event").click(function() {
        ...
        if(selectedEvent) {
            if(selectedEvent.id) {
                $("#spinner").show();
                EditCandidateShiftExtention.deleteEvent(selectedEvent.id, function(result, event) {
                    if(event.status) {
    ...
    $("#upsert-shifts").click(function() {
        if(dirtyShiftId.size > 0 && currentShifts.length > 0) {
            ...
            $("#spinner").show();
            EditCandidateShiftExtention.upsertShifts(shiftsToUpdate, function(result, event) {
